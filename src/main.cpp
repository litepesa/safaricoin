...
#if defined(NDEBUG)
# error "Safaricoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8e2bb45c59194262f48d13b9b9e17ccc514da87f5f8fa7d9a0925734681194d2");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Safaricoin: starting difficulty is 1 / 2^12
...
{
    // Safaricoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 11 blocks
    nSubsidy >>= (nHeight / 11); // Safaricoin: 11 blocks

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Safaricoin: 3 days
static const int64 nTargetSpacing = 2 * 60; // Safaricoin: 2 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Safaricoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Safaricoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5ad2d54851b70e19fcfde814055acf25aafbc54202eeba0a93b7ef37753d3ea1");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Business Daily 12/Sept/2022 Equity closes deal to acquire teachers-owned Spire Bank";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50000000 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04f2a941cab84f1827b9471ce322fe5da2548ecb15c813b723db02561af347860c7768c75dc218ff59789cb571c01e66fa09dd4d91bb065f604594495cb5ea87f1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1662979096;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085554549;

        if (fTestNet)
        {
            block.nTime    = 1662979079;
            block.nNonce   = 387309201;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xea856059dc8adcbc9dd252275633f289bf55e2f49b77a3efc6092eeee81ba409"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc6, 0xb4, 0xdf }; // Safaricoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SafaricoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SafaricoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SafaricoinMiner(CWallet *pwallet)
{
    printf("SafaricoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("safaricoin-miner");
    ...
    try { loop {
        ...
        printf("Running SafaricoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SafaricoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SafaricoinMiner, pwallet));
}
